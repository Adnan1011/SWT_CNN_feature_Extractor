function model = HC_Feats_for_Faster_RCNN_VOC2007(model)

% Stride in input image pixels at the last conv layer
model.feat_stride                               = 16;

%% Phase 1 RPN, Train RPN using transform based hand crafted features
model.rpn.solver_def_file                = fullfile(pwd, 'models', 'rpn_prototxts', 'HC_Feats', 'solver.prototxt');
...
model.fast_rcnn.solver_def_file          = fullfile(pwd, 'models', 'fast_rcnn_prototxts', 'HC_Feats', 'solver.prototxt');
model.fast_rcnn.test_net_def_file        = fullfile(pwd, 'models', 'fast_rcnn_prototxts', 'HC_Feats', 'test.prototxt');
