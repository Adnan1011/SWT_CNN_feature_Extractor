...
%% SVHN classifier
% Code for training a wavelet + shallow CNN based classifier for SVHN
% dataset
function script_CLS_HC_Feats_SVHN()
clc;
clear;
clear mex;
...
%% Script settings
dataset = fullfile(pwd, 'datasets', 'svhn', 'test');
solver_def_file = fullfile(pwd, 'models', 'CLS_SVHN_prototxts', 'solver.prototxt');
%weights_file = fullfile(pwd, 'output', 'CAE_STL_10_cachedir', 'CAE_final.caffemodel');
weights_file = fullfile(pwd, 'output', 'CLS_SVHN_cachedir', 'CLS_STL_10_iter_3000.caffemodel');
phase = 'test';
rng_seed = 7;
batch_size = 512;
...
%% init caffe solver       
cache_dir = fullfile(pwd, 'output', 'CLS_SVHN_cachedir');
...
while(iter < max_iters)
    ...
    if (~mod(iter, snapshot_interval)) && (iter ~= 0)
        snapshot(caffe_solver, cache_dir, sprintf('CLS_SVHN_iter_%d.caffemodel', iter));
    end    
    if strcmp(phase, 'train')
        iter = caffe_solver.iter();
    else
...
if strcmp(phase, 'train')
    ...
    %% Finalize
    % final snapshot
    snapshot(caffe_solver, cache_dir, 'CLS_SVHN_final.caffemodel');
else
...
function [out_images, out_labels] = augment_data(images, labels, rot_angle)
    ...
    out_images = zeros(32, 32, 3, num_images * 3, 'uint8');
    ...
    for i = aug_start : aug_end
        ...
        if rand() < 0.5
            out_images(:, :, :, i) = fliplr(images(:, :, :, mod(i - 1, num_images) + 1));
        else
            angle = randperm(rot_angle * 2, 1) - rot_angle;
            out_images(:, :, :, i) = imrotate(images(:, :, :, mod(i - 1, num_images) + 1),...
                angle, 'crop');
        end
    end
end
